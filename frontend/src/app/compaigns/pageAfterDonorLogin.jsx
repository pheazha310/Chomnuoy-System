...
export default function CampaignCard({
  ...
  const openDetail = () => navigate(`/campaigns/donor/${id}`);
  ...
  return (
    ...
    >
      <button type="button" onClick={openDetail} className="relative h-56 overflow-hidden text-left">
        <img
          src={image}
          alt={title}
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="flex-1">
          <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-primary transition-colors">
            <button type="button" onClick={openDetail} className="text-left hover:text-primary">
              {title}
            </button>
          </h3>
          <p className="text-slate-600 text-sm line-clamp-2 mb-6">{description}</p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-end text-sm">
            <div className="flex flex-col">
              ...
              <span className="text-slate-900 font-bold text-lg leading-tight">${raised.toLocaleString()}</span>
            </div>
